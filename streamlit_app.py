...
# ============ NUEVO BUSCADOR Y EDITOR ADMIN ============
def render_admin_editor():
    st.markdown('<div class="seccion">BUSCADOR Y EDICIÓN DE PUNTOS</div>', unsafe_allow_html=True)
    filtro = st.text_input("Buscar por usuario, correo, teléfono o expendiduría").lower().strip()

    def coincide(row):
        return any(filtro in str(row[col]).lower() for col in df.columns if col.lower() in ["usuario", "correo", "teléfono", "expendiduría"])

    resultados = df[df.apply(coincide, axis=1)] if filtro else df.copy()
    if resultados.empty:
        st.warning("No se encontraron coincidencias.")
        return

    for i, row in resultados.iterrows():
        with st.expander(f"{row.get('Usuario', 'Sin usuario')} - {row.get('Expendiduría', '')}"):
            nuevo_usuario = st.text_input("Usuario", value=row.get("Usuario", ""), key=f"usuario_{i}")
            nueva_pass = st.text_input("Contraseña", value=row.get("Contraseña", ""), key=f"pass_{i}")
            if st.button("Guardar cambios", key=f"guardar_{i}"):
                fila = i + 2
                worksheet.update_cell(fila, df.columns.get_loc("Usuario")+1, nuevo_usuario)
                worksheet.update_cell(fila, df.columns.get_loc("Contraseña")+1, nueva_pass)
                st.success("Cambios guardados correctamente")

# ============ ÁREA PRIVADA ============
if "auth_email" in st.session_state:
    correo_usuario = st.session_state["auth_email"]
    user = df[df["Usuario"].str.lower() == correo_usuario].iloc[0] if any(df["Usuario"].str.lower() == correo_usuario) else None
    nombre_usuario = user["Expendiduría"] if user is not None else correo_usuario
    ...
    if st.button("Acceder"):
        user = df[df["Usuario"].str.lower() == correo].iloc[0] if any(df["Usuario"].str.lower() == correo) else None
