...
html, body, .stApp { background-color: #e6e0f8; font-family: 'Montserrat', sans-serif; }
[data-testid="stToolbar"], [data-testid="stDecoration"], header, footer { display: none; }
</style>
...
def obtener_urls_imagenes(service, carpeta_id):
    try:
        ...
        return [f"https://drive.google.com/uc?id={f['id']}" for f in archivos if f['mimeType'].startswith("image/")]
...
if "auth_email" in st.session_state:
    ...
    st.markdown(f"""
    <div style="background-color:#bda2e0;padding:15px 10px;text-align:center;font-weight:bold;font-size:20px;color:black;border-radius:5px;">
    ...
    if user is None: st.error("Usuario no encontrado."); st.session_state.clear(); st.rerun()
    ...
    if st.button("Subir promociones"):
        if not imagenes: st.warning("Selecciona al menos una imagen.")
        else:
    ...
    if correo_usuario == ADMIN_EMAIL:
        columnas = [
            ...
            "Falta por entregar TAPPO", "Falta por entregar BM1000", "Ultima actualización"
        ]
    ...
    if carpeta_privada:
        carpeta_id = carpeta_privada.split("/")[-1]
        urls = obtener_urls_imagenes(creds, carpeta_id)
        if urls:
            cols = st.columns(3)
            ...
            st.warning("No se encontraron imágenes disponibles.")
    else:
        st.info("Este punto de venta todavía no tiene carpeta privada asociada.")
else:
    st.image("logo.png", use_column_width=True)
