...
if "auth_email" in st.session_state:
    ...
    if correo_usuario == ADMIN_EMAIL:
        ...
        key_unico = str(uuid.uuid4())
        opcion = st.selectbox("Selecciona un recurso para abrir:", sorted(enlaces.keys()), key=key_unico)
        if opcion:
        ...
        if termino:
            ...
            if not resultados.empty:
                ...
                seleccion = st.selectbox("Selecciona un punto para editar:", opciones, key="editar_punto")
        ...
        for col in df.columns:
            if "contraseña" not in col.lower() and col != "Carpeta privada":
